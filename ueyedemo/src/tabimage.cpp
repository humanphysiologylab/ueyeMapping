...
void properties::UpdateMasterGainCtrls ()
{
    ...
    /* maybe auto framerate is set externally, therefore we have to set gui state here.
     * (e.g. by ini file.)
     * (unfortunately we can not do this  for xs because the xs tab will need to overwrite this checkbox for auto gain shutter) */
    if (!m_bIsXs)
    {
    ...
    else
    {
        ...
        {
            is_SetAutoParameter (m_hCamera, IS_GET_ENABLE_AUTO_SENSOR_GAIN_SHUTTER, &dblEnable, NULL);
            m_bAutoGain = (dblEnable != 0.0);
        }
    }
