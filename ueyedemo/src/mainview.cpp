...
void Mainview::onSaveImage ()
{
	...
	{
		is_AllocImageMem(m_hCamera, width, height, depth, &(pcImageMem[i]), &(id[i]));
		is_AddToSequence(m_hCamera, pcImageMem[i], id[i]);
		is_SetAllocatedImageMem(m_hCamera, width, height, depth, pcImageMem[i], &(id[i]));
	}
	long long diff;
	...
	while(go_on)
	{
		clock_gettime(CLOCK_REALTIME, &currentspec); 
                diff = (currentspec.tv_sec - startspec.tv_sec) * 1.0e9 + (currentspec.tv_nsec - startspec.tv_nsec);
		if (diff > timeout)
		{
			go_on = 0;
		}
