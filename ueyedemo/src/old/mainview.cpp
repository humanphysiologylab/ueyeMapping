 ...
 * \li  12.12.2014 - (1.14.8) - Patch 'QAction' hw trigger to sw trigger if camera is a XC
...
void Mainview::onSaveImage ()
{
	...
	printf("GetImageMem returned %d\n", ret);

	ret = is_GetImageMem(m_hCamera, &pcImageMem);

	printf("GetImageMem returned %d\n", ret);

	int size = (width * int((depth + 7)/8))*height;

	printf("Size of the active memory: %d bytes\n", size);
	...
	bool go_on = 1;
	//double newFPS;
	//ret = is_SetFrameRate(m_hCamera, 1000, &newFPS);
	//printf("SetFR returned %d, new framerate is %d\n", ret, newFPS);
        ...
        while(go_on)
        {
            ...
            {	
                timeHasCome = 0;
		memcpy(frames[iterator], pcImageMem, size);
		iterator++;
                printf("%d\n", iterator);
            }
            else
            {
                ...
                long long diff = (currentspec.tv_sec - startspec.tv_sec) * 1.0e9 + (currentspec.tv_nsec - startspec.tv_nsec);
                if (diff > (1000000 * (iterator+1)))
                {
                    timeHasCome = 1;
                }
		if (diff > 1.0e10)
	...
	printf("Done\n");
	ofstream binary_stream("sequence.bin", ios::binary);
	for(int i = 0; i < 10000; i++)
	{
		binary_stream.write(reinterpret_cast<char const *>(frames[i]), size);
	}
    }
    else
    {
