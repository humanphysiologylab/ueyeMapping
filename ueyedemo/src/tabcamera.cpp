...
void properties::UpdateExposureCtrls ()
{
    ...
    {
        /* XS/XC cameras --> check auto sensor features */
        if (m_bXsHasAES && (is_SetAutoParameter(m_hCamera, IS_GET_ENABLE_AUTO_SENSOR_SHUTTER, &dblEnable, NULL) == IS_SUCCESS))
        {
            m_bAutoExposure = (dblEnable != 0.0);
        }

        if (m_bXsHasAGES && (is_SetAutoParameter(m_hCamera, IS_GET_ENABLE_AUTO_SENSOR_GAIN_SHUTTER, &dblEnable, NULL) == IS_SUCCESS))
        {
            m_bAutoExposure = (dblEnable != 0.0);
        }
    }
    ...
    checkAutoExp->setEnabled((m_bHasAutoExposure || m_bXsHasAES || m_bXsHasAGES) && !m_bMaxExposure);
    // max
...
void properties::checkAutoExp_clicked ()
{
    ...
    if (m_bIsXs)
    {
        /* XC/XS use sensor auto features */
        if (m_bXsHasAES)
        {
